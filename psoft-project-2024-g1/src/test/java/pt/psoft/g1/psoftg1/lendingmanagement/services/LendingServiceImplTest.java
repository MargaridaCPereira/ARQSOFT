...
class LendingServiceImplTest {
        ...
        @InjectMocks
        private LendingServiceImpl lendingServiceImpl; // Mocks serão injetados aqui
        ...
        void testFindByLendingNumber() {
                when(lendingRepository.findByLendingNumber(LocalDate.now().getYear() + "/999"))
                                .thenReturn(Optional.of(lending));
                assertThat(lendingServiceImpl.findByLendingNumber(LocalDate.now().getYear() + "/999")).isPresent();
                assertThat(lendingServiceImpl.findByLendingNumber(LocalDate.now().getYear() + "/1")).isEmpty();
        }
        /*@Test
        void testCreate() {
                var request = new CreateLendingRequest("978-3-16-148410-0", // Use a valid ISBN
                                LocalDate.now().getYear() + "/1");

                when(lendingRepository.save(any())).thenReturn(lending); // Simulate save

                var lending1 = lendingServiceImpl.create(request);
                assertThat(lending1).isNotNull();

                // Create mock returns for subsequent creations
                when(lendingRepository.save(any())).thenReturn(lending);
                var lending2 = lendingServiceImpl.create(request);
                assertThat(lending2).isNotNull();

                var lending3 = lendingServiceImpl.create(request);
                assertThat(lending3).isNotNull();

                // 4th lending - simulate exception
                assertThrows(LendingForbiddenException.class, () -> lendingServiceImpl.create(request));
        ...
        void testSetReturned() {
                ...
                assertThrows(StaleObjectStateException.class,
                                () -> lendingServiceImpl.setReturned(year + "/" + seq, request,
                                                (notReturnedLending.getVersion() - 1)));

                assertDoesNotThrow(
                                () -> lendingServiceImpl.setReturned(year + "/" + seq, request,
                                                notReturnedLending.getVersion()));
        } */
        ...
        void testRecommendBooksForChild() {
                ...
                List<Book> recommendedBooks = lendingServiceImpl.recommendBooks(1L, 1); // Mudei para 1 aqui
        ...
        void testRecommendBooksForTeen() {
                ...
                List<Book> recommendedBooks = lendingServiceImpl.recommendBooks(1L, 1); // Recomendando 1 livro
        ...
        void testRecommendMostLentBooks() {
                ...
                when(lendingServiceImpl.getTopGenres(Y)).thenReturn(topGenres);
                ...
                when(lendingServiceImpl.getTopLentBooksByGenre("fiction", X)).thenReturn(fictionBooks);
                when(lendingServiceImpl.getTopLentBooksByGenre("non-fiction", X)).thenReturn(nonFictionBooks);
                when(lendingServiceImpl.getTopLentBooksByGenre("children", X)).thenReturn(childrenBooks);

                // Executando o método de recomendação
                List<Book> recommendedBooks = lendingServiceImpl.recommendMostLentBooks(X, Y);
